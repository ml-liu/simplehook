...
static int funchook_prepare_internal_jit(funchook_t *funchook, void **target_func);
...
int funchook_prepare_jit(funchook_t *funchook, void **target_func)
{
    int rv;
    void *orig_func;

    funchook_log(funchook, "Enter funchook_prepare(%p, %p)\n", funchook, target_func);
    orig_func = *target_func;
    rv = funchook_prepare_internal_jit(funchook, target_func);
    funchook_log_end(funchook, "Leave funchook_prepare(..., [%p->%p],...) => %d\n", orig_func, *target_func, rv);
    return rv;
}
...
static int funchook_prepare_internal_jit(funchook_t *funchook, void **target_func)
{
    ...
    rv = funchook_make_trampoline(funchook, disp, func, trampoline);
	 ...
	 funchook_log(funchook, " first long trampoline\n");
    funchook_log_trampoline(funchook, trampoline);
    if (rv != 0) {
    ...
    entry->hook_func = NULL;
    memcpy(entry->trampoline, trampoline, TRAMPOLINE_SIZE);
    memcpy(entry->old_code, func, JUMP32_SIZE);

	extern void* alloc_wrap(void* func, const char* sig);
	
	void* new_fun_ptr = alloc_wrap(entry->trampoline, NULL);
	
#ifdef CPU_X86_64
    if (funchook_jump32_avail(func, new_fun_ptr)) {
        funchook_write_jump32(funchook, func, new_fun_ptr, entry->new_code);
        entry->transit[0] = 0;
    } else {
        funchook_write_jump32(funchook, func, entry->transit, entry->new_code);
        funchook_write_jump64(funchook, entry->transit, new_fun_ptr);
    }
#else
    funchook_write_jump32(funchook, func, new_fun_ptr, entry->new_code);
#endif
...
int funchook_prepare_internal(funchook_t *funchook, void **target_func, void *hook_func)
{
    void *func = *target_func;
    uint8_t trampoline[TRAMPOLINE_SIZE];
    rip_displacement_t disp[2] = {{0,},{0,}};
