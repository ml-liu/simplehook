...
def main(pid1, pid2):
    btmap = load_bt_map(pid2)
    # print btmap
    fg = CallStack()
    target_ls = None
    line_cnt = 0
    print "begin load file %s" %(pid1)
    stat_map_1 =  readfile(pid1)
    print "begin load file %s" %(pid2)
    stat_map_2 =  readfile(pid2)
    ...
    for i in stat_map_2:
        if i in stat_map_1:
            add_size = stat_map_2[i] - stat_map_1[i]
            if add_size > 0:
                real_map[i] = add_size
        else:
            real_map[i] = stat_map_2[i]
    ...
    with open("diffadd.%s-%s.html" % (pid1, pid2), "wb") as fp:
        fp.write(fg.Print())
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        print "python ./diffadd.py file1 file2"
