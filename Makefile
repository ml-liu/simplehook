...
simplehook.so: main.c
	gcc -c  -m64 -Wno-missing-braces -D_GNU_SOURCE -DPFB_NO_EXTERNAL_FUNC -Ifunhook/include -Ifunhook/distorm/include -fPIC queue.c common.c  funhook/src/os_func.c funhook/src/os_func_unix.c funhook/src/funchook_syscall.S funhook/src/printf_base.c funhook/src/funchook.c funhook/src/funchook_io.c funhook/src/funchook_x86.c funhook/src/funchook_unix.c funhook/distorm/src/mnemonics.c funhook/distorm/src/wstring.c funhook/distorm/src/textdefs.c funhook/distorm/src/prefix.c funhook/distorm/src/operands.c funhook/distorm/src/insts.c funhook/distorm/src/instructions.c funhook/distorm/src/distorm.c funhook/distorm/src/decoder.c
	ar -r libcommon.a common.o decoder.o distorm.o funchook_io.o funchook.o funchook_syscall.o funchook_unix.o funchook_x86.o instructions.o insts.o mnemonics.o operands.o os_func.o os_func_unix.o prefix.o printf_base.o queue.o textdefs.o wstring.o
	...
	g++ -fpermissive -Ifunhook/include -g lua_table_plugin.cpp -o liblua_table_plugin.so --shared -fPIC libcommon.a
